...
class networkTabularQAgent(object):
    ...
    def learn3(self,iter):
        ...
        _ ,loss,log_probs,act_val,l1=self.sess.run([self.train_op,self.loss,self.neg_log_prob,self.all_act,self.layer], feed_dict={
    ...
    def learn5(self,iter):
        ...
        self.ep_obs2=np.array(self.ep_obs).reshape(l,self.n_features)
        discounted_ep_rs_norm = self._discount_and_norm_rewards()
        X_batch, Y_batch, Z_batch = self.next_minibatch(np.vstack(self.ep_obs2), np.array(self.ep_as), np.array(discounted_ep_rs_norm),l)
        _ ,loss,log_probs,act_val=self.sess.run([self.train_op,self.loss,self.neg_log_prob,self.all_act], feed_dict={
    ...
    def learn4(self,iter):
        ...
        self.ep_obs2=np.array(self.ep_obs).reshape(l,self.n_features)
        for i in range(0,1):
            X_batch, Y_batch, Z_batch = self.next_minibatch(np.vstack(self.ep_obs2), np.array(self.ep_as), np.array(self.ep_rs),l)
            self.sess.run(self.train_op, feed_dict={
                self.tf_obs: X_batch,  # shape=[None, n_obs]
                self.tf_acts: Y_batch,  # shape=[None, ]
                self.tf_vt:  Z_batch, # shape=[None, ]
